...
// Current authenticated user's ID (set on login, used for multi-tenancy)
...
async function _sbUpsert(table, records) {
  ...
  const withUser = records.map(r => ({ ...r, user_id: _currentUserId }));
...
async function _sbSetting(key, value) {
  const { error } = await supabaseClient.from('settings').upsert({ user_id: _currentUserId, key, value });
...
async function loadSampleData() {
  const { data } = await supabaseClient.from('settings').select('value').eq('key', 'seeded').eq('user_id', _currentUserId).maybeSingle();
  if (data) return;
...
function renderProfile() {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="page-body">
      ...
      <div class="card mobile-signout-card">
        <div class="card-body" style="display:flex;align-items:center;justify-content:space-between;gap:12px">
          <div>
            ...
            <div style="font-size:12px;color:var(--gray-400)" id="profile-user-email"></div>
          </div>
          <button class="btn btn-danger" onclick="logoutUser()" style="white-space:nowrap">Sign Out</button>
        </div>
      </div>

    </div>`;

  // Populate signed-in email
  supabaseClient.auth.getUser().then(({ data: { user } }) => {
    const el = document.getElementById('profile-user-email');
...
async function initApp() {
  ...
  // Store user ID for multi-tenancy (all saves will be scoped to this user)
